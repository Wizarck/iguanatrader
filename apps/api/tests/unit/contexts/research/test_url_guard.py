...
def _fake_getaddrinfo(ip: str):
    def _inner(host, port, *args, **kwargs):  # noqa: ANN001, ANN002
...
def test_rejects_public_host_resolving_to_private(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(url_guard.socket, "getaddrinfo", _fake_getaddrinfo("192.168.1.10"))
    with pytest.raises(UnsafeUrlError):
...
def test_rejects_ipv4_mapped_private_v6(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(url_guard.socket, "getaddrinfo", _fake_getaddrinfo("::ffff:10.0.0.1"))
    with pytest.raises(UnsafeUrlError):
