...
from iguanatrader.contexts.trading.fills_reconciliation import FillsReconciliationService
    ...
    * ``strategy_resolver``: a callable that maps a
    ...
    def __init__(
        ...
        *,
        ...
        # Fills/equity reconciliation (drain broker fills, close trades,
        # snapshot equity) is extracted to its own cohesive service; this
        # orchestrator delegates to it. Same collaborators it referenced when
        # the bodies lived here.
        self._fills_reconciler = FillsReconciliationService(bus=bus, broker=broker)
    ...
    async def startup_reconcile(
        ...
        *,
        ...
        """Drain broker fills the daemon missed while it was down.

        Thin delegator to :class:`FillsReconciliationService` (the body was
        extracted there). Retained on ``TradingService`` because the CLI
        startup + daemon-lifecycle reconcile reach it through the service.
        """
        await self._fills_reconciler.startup_reconcile(safety_margin_minutes=safety_margin_minutes)

    async def reconcile_fills_handler(self, since: datetime) -> None:
        """Drain ``BrokerPort.reconcile_fills(since)``.

        Thin delegator to :class:`FillsReconciliationService`.
        """
        await self._fills_reconciler.reconcile_fills_handler(since)
    ...
    async def _reconcile_one_fill(
        ...
        *,
        ...
        """Process a single :class:`FillEvent`.

        Thin delegator to
        :meth:`FillsReconciliationService.reconcile_one_fill`; retained so
        existing callers/tests that reach it through the service keep working.
        """
        await self._fills_reconciler.reconcile_one_fill(
            fill_event,
            fill_repo=fill_repo,
