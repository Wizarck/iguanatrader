...
- **`scripts/iguana-compose.sh` overlay-stack** — currently loads `mvp` + `mvp.override` + `postgres` + `ibgateway` for paper/live. After slice 5 (`compose/backup.yml`) merged, the wrapper does NOT yet include the backup overlay. One-line fix in the resilient-presence pattern; track as a `housekeeping` follow-up.
