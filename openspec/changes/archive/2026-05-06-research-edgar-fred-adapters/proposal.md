...
- **Idempotency via `research_facts.dedupe_key`** — each adapter builds a deterministic key (`<source>:<filing_id>` for EDGAR, `<source>:<series_id>:<observation_date>:<vintage>` for FRED/ALFRED, `<source>:<series_id>:<period>:<release_date>` for BLS, `<source>:<dataset>:<table>:<frequency>:<period>:<release_date>` for BEA). The migration shipping in this slice (`0004_research_dedupe_index.py` per design D7) adds a nullable `dedupe_key TEXT` column to `research_facts` plus a partial unique index on `(tenant_id, dedupe_key) WHERE dedupe_key IS NOT NULL`. On insert, the adapter pre-checks via `repo.find_by_dedupe_key(tenant_id, key)`; a hit short-circuits to no-op + emits `research.<source>.skipped_duplicate`. Re-running an adapter the day after a fresh ingest is a no-op.
