...
- [x] 19. **SOPS env split** (PR #268, 2026-05-18) — added `IBKR_USERNAME_LIVE`, `IBKR_PASSWORD_LIVE`, `IBKR_ACCOUNT_ID_LIVE` to `.secrets/live.env.enc` (aliases of the unsuffixed keys, kept for backwards-compat). Also corrected `.secrets/paper.env.enc` to use the IBKR portal login (`arturoramirez6`) instead of the auto-generated paper-trading user — gnzsnz/ib-gateway authenticates against the portal. Live account ID `U12492989` populated. Method: explicit `--age <recipient>` flag on encrypt (the `.sops.yaml` path_regex resolver fails for temp paths outside the repo). Roundtrip-verified.
