...
sources:
  ...
  - .ai-playbook/docs/concepts/bmad-openspec-bridge.md §3.1 (canonical schema)
  - .ai-playbook/docs/concepts/runbook-bmad-openspec.md §2.4 (slicing) + §3.1 (per-change sequence)
---
...
3. **OpenSpec lifecycle** (per `.ai-playbook/docs/concepts/runbook-bmad-openspec.md` §3.1):
...
## Net-new slices (beyond the original catalogue)

Slices added after the initial 20-change plan, with their dependencies.

| id | context | depends on | summary |
|----|---------|-----------|---------|
| `mcp-hitl-approvals` | approval + api | `approval-channels-multichannel` (P1), MCP surface (B) | Expose owner-only HITL action tools (approve/reject/halt/resume/lock/unlock + list-pending) on the MCP REST surface for the external Hermes gateway, gated by per-operator `authorized_senders` revalidation; enriched approval push + execution/close-out follow-ups. See [openspec/changes/mcp-hitl-approvals/](../openspec/changes/mcp-hitl-approvals/) + [docs/mcp-hitl-approvals-deploy.md](mcp-hitl-approvals-deploy.md). |
...
- [.ai-playbook/docs/concepts/bmad-openspec-bridge.md](../.ai-playbook/docs/concepts/bmad-openspec-bridge.md) §3.1 — canonical schema this file follows
- [.ai-playbook/docs/concepts/runbook-bmad-openspec.md](../.ai-playbook/docs/concepts/runbook-bmad-openspec.md) §3 — OpenSpec lifecycle each slice follows
