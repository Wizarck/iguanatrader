...
| **CD / deployment automation** | **NO auto-deploy a live** desde día 1 (constraint AGENTS.md §4). **dev**: `docker compose up` local. **paper/live**: TBD — k3s + Rancher Manager + Fleet GitOps es el target estratégico (mismo patrón que eligia-core/helm/eligia-stack), pero el wiring concreto (Helm charts + GitRepo CR) lo aterriza una future slice `deployment-foundation` que helmifica todos los componentes juntos (api + openbb-sidecar + litestream + frontend), no piecemeal por slice. Hasta entonces, `docker-compose.{paper,live}.yml` siguen siendo placeholders válidos como rehearsal local. Decisión 2026-05-06 (slice R4 retro): R4 ship-eó un Helm chart prematuramente que se revirtió en favor de container-only (la license boundary se preserva igual a nivel de contenedor, sea compose o k8s) |
...
| Aspecto | Resolución |
|---|---|
| **Deployment unit** | Container con propio `pyproject.toml`/Dockerfile en `apps/openbb-sidecar/`. **dev**: docker-compose service `openbb_sidecar` (no host port — `expose:` only). **paper/live**: container-only por ahora; k8s Pod vía Helm chart cuando aterrice la `deployment-foundation` slice (post-R4). Deps OpenBB AGPL viven solo en este container; nunca en `apps/api/` |
| **Communication** | `http://openbb_sidecar:8765` (compose internal DNS, no host binding). En paper/live cuando se k8s-ifique mantendrá la misma URL pattern via Service DNS dentro del namespace `iguanatrader` (defensa en profundidad: solo el monolito habla con el sidecar) |
| **iguanatrader client** | `contexts/research/sources/openbb_sidecar.py` usa `httpx.Client`. URL via env `OPENBB_SIDECAR_URL` (default `http://openbb_sidecar:8765`). Mismo cliente sirve para compose-DNS dev y k8s-Service paper/live cuando llegue |
| **Resource limits target** | Para futura slice de Helm: `requests: {cpu: 500m, memory: 1Gi}`, `limits: {cpu: 1, memory: 2Gi}` — basado en perfil idle ~400-600 MB + peaks ~1.5 GB en heavy queries. Documentado aquí para que la deployment-foundation slice los reuse |
| **VPS impacto** | Hetzner CAX31 actual (16 GB RAM, 8 vCPU): cómodo, sin upgrade necesario. ~10% extra resource consumption |
| **Liveness** | docker `HEALTHCHECK` HTTP-GET `/health` en el container (compose). En paper/live k8s: `livenessProbe`/`readinessProbe` con la misma URL. El monolito client emite estructurado log retry/error pattern; OTel-gauge integration vendrá con la observability slice |
