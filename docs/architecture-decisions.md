...
| **CD / deployment automation** | **NO auto-deploy a live** desde día 1 (constraint AGENTS.md §4). **dev**: `docker compose up` local. **paper/live**: Helm charts en `helm/<bundle>/` desplegados a k3s vía Rancher Manager + Fleet (GitOps polling). CI build images en GHCR; promotion manual: bump `image.tag`/`image.digest` en `values-vps.yaml` + push → Fleet detecta + reconcilia. Bootstrap CR en `deploy/fleet-gitrepo.yaml` (one-shot apply via SSH al management cluster). Decisión cerrada 2026-05-06 con R4 — primera slice deployable. v3 SaaS sigue mismo patrón Helm+Fleet, expansión a multi-cluster |
...
| Aspecto | Resolución |
|---|---|
| **Deployment unit** | Container con propio `pyproject.toml`/Dockerfile en `apps/openbb-sidecar/`. **dev**: docker-compose service `openbb_sidecar` (no host port). **paper/live**: k8s Pod via Helm chart `helm/openbb-sidecar/` desplegado por Rancher Fleet (GitOps). Deps OpenBB AGPL viven solo en este container; nunca en `apps/api/` |
| **Communication** | **dev**: `http://openbb_sidecar:8765` (compose internal DNS, no host binding). **paper/live**: ClusterIP Service `http://openbb-sidecar:8765` resuelve via cluster DNS solo desde Pods en namespace `iguanatrader`. NetworkPolicy ingress restringido al label `app.kubernetes.io/component=api` (defensa en profundidad: solo el monolito habla con el sidecar) |
| **iguanatrader client** | `contexts/research/sources/openbb_sidecar.py` usa `httpx.AsyncClient`. URL via env `OPENBB_SIDECAR_URL` (default `http://openbb_sidecar:8765`). Mismo cliente sirve para compose-DNS dev y k8s-Service paper/live |
| **Resource limits k3s** | Helm `values-vps.yaml`: `requests: {cpu: 500m, memory: 1Gi}`, `limits: {cpu: 1, memory: 2Gi}` — basado en perfil idle ~400-600 MB + peaks ~1.5 GB en heavy queries |
| **VPS impacto** | Hetzner CAX31 actual (16 GB RAM, 8 vCPU): cómodo, sin upgrade necesario. ~10% extra resource consumption |
| **Liveness** | `livenessProbe`/`readinessProbe` HTTP-GET `/health` en el container (k8s); `HeartbeatMixin` en el monolito client emite OTel gauge `iguana_openbb_sidecar_heartbeat_last_ts` cada 30s; Eligia alerta si gap > 90s |
